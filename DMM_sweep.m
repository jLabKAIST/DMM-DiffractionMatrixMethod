function [result] = DMM_sweep(param,up,rd,ru,tu)
...
for uxindex = 1:Diffsize
    ...
    if (uxindex-1)*resolution+z<=lengthofux
        ...
        theta_air(z,uxindex,uyindex) = asind(sind(theta(z,uxindex,uyindex))*n_org/n_glass);
        theta_air(z,uxindex+Diffsize,uyindex) = asind(sind(theta(z,uxindex,uyindex))*n_org/n_glass);

        Expe = exp(-1i*k0*n_org*x_dipole*ux);
        
        % E field ( x-dipole )
        if Dox == 1
            Eplusx(z,uxindex,uyindex) = E_Coefficient * uy/uz/ull * Expe;
            Eplusx(z,uxindex+Diffsize,uyindex) = -E_Coefficient * ux/ull * Expe;
            Eminusx(z,uxindex,uyindex) = E_Coefficient * uy/uz/ull * Expe;
            Eminusx(z,uxindex+Diffsize,uyindex) = E_Coefficient * ux/ull * Expe;
        end
        % E field ( y-dipole )
        if Doy == 1
        ...
        %
        % Diagonal matrix, diagonal element : TE->TE 0th or TM->TM 0th
...
for Orgindex = 1:length(param.sweeprange.Org)
for Dipindex = 1:length(param.sweeprange.Dip)

    Org_thickness = param.sweeprange.Org(Orgindex);
    Dipole_pos = param.sweeprange.Dip(Dipindex);
...
if Dox == 1
    ...
    if param.singledip ==1
        Ioutx(z,:,uyindex) = abs(Eoutx(z,:,uyindex)).^2 .* n_glass/n_org .* cosd(theta_air(z,:,uyindex)) ./ cosd(theta(z,:,uyindex))*  I_Coefficient .* max(0,cosd(theta_air(z,:,uyindex)).^2);     
    else
        Ioutx(z,:,uyindex) = sum(C_ITO,1) .* n_glass/n_org .* cosd(theta_air(z,:,uyindex)) ./ cosd(theta(z,:,uyindex)).* I_Coefficient .* max(0,cosd(theta_air(z,:,uyindex)).^2);
    end
...
if Doy == 1
    ...
    if param.singledip ==1
        Iouty(z,:,uyindex) = abs(Eouty(z,:,uyindex)).^2 .* n_glass/n_org .* cosd(theta_air(z,:,uyindex)) ./ cosd(theta(z,:,uyindex))*  I_Coefficient .* max(0,cosd(theta_air(z,:,uyindex)).^2);     
    else
        Iouty(z,:,uyindex) = sum(C_ITO,1) .* n_glass/n_org .* cosd(theta_air(z,:,uyindex)) ./ cosd(theta(z,:,uyindex)).* I_Coefficient .* max(0,cosd(theta_air(z,:,uyindex)).^2);
    end
...
if Doz == 1
    ...
    if param.singledip ==1
        Ioutz(z,:,uyindex) = abs(Eoutz(z,:,uyindex)).^2 .* n_glass/n_org .* cosd(theta_air(z,:,uyindex)) ./ cosd(theta(z,:,uyindex))*  I_Coefficient .* max(0,cosd(theta_air(z,:,uyindex)).^2);     
    else
        Ioutz(z,:,uyindex) = sum(C_ITO,1) .* n_glass/n_org .* cosd(theta_air(z,:,uyindex)) ./ cosd(theta(z,:,uyindex)).* I_Coefficient .* max(0,cosd(theta_air(z,:,uyindex)).^2);
    end
...
for uxindex = 1:Diffsize
    for uyindex = 1:length(uysize)
        if (uxindex-1)*resolution+z<=lengthofux
            ...
            Expe2 = exp(1i*k0*n_org*x_dipole*ux); %%% Position at the dipole
...
for uxindex = 1:Diffsize
    for uyindex = 1:length(uysize)
        if (uxindex-1)*resolution+z<=lengthofux
            ...
            ull = sqrt(ux^2+uy^2);
            uz = sqrt(1-ull^2);
            uz_air = sqrt(1- (n_org/n_glass)^2 * ull^2);
            if abs(uz) < 1e-8
                uz = 0;
            end
...
[a,b] = min(abs(uxsize-n_glass/n_org));
[c,d] = min(abs(uxsize+n_glass/n_org));
...
result.xdip.LEE(Orgindex,Dipindex) = LEEx;
result.ydip.LEE(Orgindex,Dipindex) = LEEy;
result.zdip.LEE(Orgindex,Dipindex) = LEEz;
result.tdip.LEE(Orgindex,Dipindex) = LEE;

result.xdip.Purcell(Orgindex,Dipindex) = Purcellx;
result.ydip.Purcell(Orgindex,Dipindex) = Purcelly;
result.zdip.Purcell(Orgindex,Dipindex) = Purcellz;
result.tdip.Purcell(Orgindex,Dipindex) = PurcellA;

result.xdip.T(Orgindex,Dipindex) = Transx;
result.ydip.T(Orgindex,Dipindex) = Transy;
result.zdip.T(Orgindex,Dipindex) = Transz;
result.tdip.T(Orgindex,Dipindex) = Trans;

result.xdip.K(Orgindex,Dipindex).data = Koutnormx;
result.ydip.K(Orgindex,Dipindex).data = Koutnormy;
result.zdip.K(Orgindex,Dipindex).data = Koutnormz;
result.tdip.K(Orgindex,Dipindex).data = Ksum;

result.xdip.I(Orgindex,Dipindex).data = Ioutnormx;
result.ydip.I(Orgindex,Dipindex).data = Ioutnormy;
result.zdip.I(Orgindex,Dipindex).data = Ioutnormz;
result.tdip.I(Orgindex,Dipindex).data = Isum;

result.xdip.L(Orgindex,Dipindex).data = Lxyx;
result.ydip.L(Orgindex,Dipindex).data = Lxyy;
result.zdip.L(Orgindex,Dipindex).data = Lxyz;
result.tdip.L(Orgindex,Dipindex).data = Lsum;

result.Ifin(Orgindex,Dipindex).data = Ifin;
